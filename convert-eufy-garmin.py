...
def convert_to_utc_datetime(date_str, time_str):
    ...
    dt_local = datetime.strptime(f"{clean_date}", "%Y/%m/%f %H:%M:%S")
    return dt_local
...
for _, row in df.iterrows():
    try:
        dt = convert_to_utc_datetime(row['Time'])
        ...
        encoder.write_mesg({
            ...
            'metabolic_age': int(float(row['METABOLIC AGE'])) if row['METABOLIC AGE'] else '--',
            'bmi': float(row['BMI']),
